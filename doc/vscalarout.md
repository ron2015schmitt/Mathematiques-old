...
_This document was automatically generated from file_ **`vscalarout.cpp`** (mātricks-v2.16-r20).
...
```C++
  integrate_a2b(v,-1.,1.) = double; 
  integrate_a2b(v,-1.,1.,2) = double; 
  integrate_a2b(v,-1.,1.,3) = double; 
  integrate_a2b(v,-1.,1.,4) = double; 
```
...
```C++
  ...
  min(v) = double; 
  2*exp(v)+1 = {double, double, double, double}; 
  min(2*exp(v)+1) = double; 
```
