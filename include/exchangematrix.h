#ifndef MATHQ__EXCHANGEMATRIX_H
#define MATHQ__EXCHANGEMATRIX_H 1
...
namespace mathq {
  ...
  /********************************************************************
   * ExchangeMatrix<D        -- variable size matrix (valarray)
   *                        D  = type for elements
   * ExchangeMatrix<D,NR>    -- fixed number of rows (valarray)
   *                        NR = number of rows
   * ExchangeMatrix<D,NR,NC> -- fixed number of rows and cols (array)
   *                        NC = number of cols
   ********************************************************************  
   */
  ...
  class ExchangeMatrix : public TensorRW<ExchangeMatrix<D,NR,NC>,D,D,1,2>{
  ...
  public:
    ...
    typedef ExchangeMatrix<D,NR,NC> XType;
    typedef D EType;
    typedef D DType;
  ...
  private:
    ...
    static_assert(NumberType<D>::value,
                  "class ExchangeMatrix can only have numbers as elements, ie not vectors, matrices etc.");
  ...
  public:
    ...
    explicit ExchangeMatrix<D,NR,NC>() 
    {
      size_t NN = NR*NC;
      resize(NR,NC);
    }
    ...
    explicit ExchangeMatrix<D,NR,NC>(const size_type Nr, const size_type Nc) {
      resize(Nr,Nc);
    }
    ...
    ExchangeMatrix<D,NR,NC>&  resize(const int Nr, const int Nc) {
      Nrows_ = NR;
      Ncols_ = NC;
    ...
    ExchangeMatrix<D,NR,NC>& resize(const Dimensions dims) {
    ...
    ExchangeMatrix<D,NR,NC>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    ExchangeMatrix<D,NR,NC>& reshape(const size_type nr, const size_type nc) { 
    ...
    ExchangeMatrix<D,NR,NC>& transpose(void) { 
      return *this;
    }
    ...
    typename std::enable_if<is_complex<T>{}, ExchangeMatrix<D,NR,NC>& >::type adjoint() {
      return *this;
    }
    ...
    ExchangeMatrix<D,NR,NC>&  roundzero(FType tolerance = Helper<FType>::tolerance) {
      return *this;
    }
  ...
  // NOTE: in-place
    ...
    template< typename T=D >
    typename std::enable_if<is_complex<T>{},  ExchangeMatrix<D,NR,NC>& >::type conj() {
      return *this;
    }
  ...
  friend std::ostream& operator<<(std::ostream &stream, const ExchangeMatrix<D,NR,NC>& m) {
    using namespace display;
  ...
  //template <class D>	
  friend inline std::istream& operator>>(const std::string s,  ExchangeMatrix<D,NR,NC>& m2) {	
  ...
  friend std::istream& operator>>(std::istream& stream,  ExchangeMatrix<D,NR,NC>& m2) {	
    return stream;
  }
