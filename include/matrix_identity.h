...
namespace mathq {
  ...
  /********************************************************************
   * MatrixIdentity<D        -- variable size matrix (valarray)
   *                        D  = type for elements
   * MatrixIdentity<D,NR>    -- fixed number of rows (valarray)
   *                        NR = number of rows
   * MatrixIdentity<D,NR,NC> -- fixed number of rows and cols (array)
   *                        NC = number of cols
   ********************************************************************  
   */
  ...
  class MatrixIdentity : public TensorRW<MatrixIdentity<D,NR,NC>,D,D,1,2>{
  ...
  public:
    ...
    typedef MatrixIdentity<D,NR,NC> XType;
    typedef D EType;
    typedef D DType;
  ...
  public:
    ...
    explicit MatrixIdentity<D,NR,NC>() 
    {
      size_t NN = NR*NC;
      resize(NR,NC);
    }
    ...
    explicit MatrixIdentity<D,NR,NC>(const size_type Nr, const size_type Nc) {
      resize(Nr,Nc);
    }
    ...
    MatrixIdentity<D,NR,NC>&  resize(const int Nr, const int Nc) {
      Nrows_ = NR;
      Ncols_ = NC;
    ...
    MatrixIdentity<D,NR,NC>& resize(const Dimensions dims) {
    ...
    MatrixIdentity<D,NR,NC>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    MatrixIdentity<D,NR,NC>& reshape(const size_type nr, const size_type nc) { 
    ...
    MatrixIdentity<D,NR,NC>& transpose(void) { 
      return *this;
    }
    ...
    typename std::enable_if<is_complex<T>{}, MatrixIdentity<D,NR,NC>& >::type adjoint() {
      return *this;
    }
    ...
    MatrixIdentity<D,NR,NC>&  roundzero(FType tolerance = Functions<FType>::tolerance) {
      return *this;
    }
  ...
  // NOTE: in-place
    ...
    template< typename T=D >
    typename std::enable_if<is_complex<T>{},  MatrixIdentity<D,NR,NC>& >::type conj() {
      return *this;
    }
  ...
  friend std::ostream& operator<<(std::ostream &stream, const MatrixIdentity<D,NR,NC>& m) {
    using namespace display;
  ...
  //template <class D>	
  friend inline std::istream& operator>>(const std::string s,  MatrixIdentity<D,NR,NC>& m2) {	
  ...
  friend std::istream& operator>>(std::istream& stream,  MatrixIdentity<D,NR,NC>& m2) {	
    return stream;
  }
