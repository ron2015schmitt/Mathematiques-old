#ifndef MATHQ__MATRIX_LOWERTRIANGLE_H
#define MATHQ__MATRIX_LOWERTRIANGLE_H 1
...
namespace mathq {
  ...
  /********************************************************************
   * MatrixLowerTriangle<D>    -- variable size matrix (valarray)
   *                              D  = type for elements
   * MatrixLowerTriangle<D,N>  -- fixed number of rows (valarray)
   *                              N = number of rows
   *                                = number of cols
   ********************************************************************  
   */
  ...
  class MatrixLowerTriangle : public TensorRW<MatrixLowerTriangle<D,N>,D,D,1,2>{
  ...
  public:
    ...
    // -------------------  DEFAULT  CONSTRUCTOR --------------------
    explicit MatrixLowerTriangle<D,N>() 
    {
      resize(N);
      *this = 0;
    }
    ...
    // -------------------  D value --------------------
    explicit MatrixLowerTriangle<D,N>(const D& value) 
    {
      resize(N);
      *this = value;
    }
    ...
    explicit MatrixLowerTriangle<D,N>(const Vector<D>& v) 
    {
    ...
    explicit MatrixLowerTriangle<D,N>(const TensorR<X,D,D,1,2> A) {
    ...
    explicit MatrixLowerTriangle<D,N>(const size_type M) {
      resize(M);
      *this = 0;
    }
    ...
    explicit MatrixLowerTriangle<D,N>(const size_type M, const D& value) {
      resize(M);
      *this = value;
    }
    ...
    MatrixLowerTriangle<D,N>&  resize(const int M) {
      N_ = N;
    ...
    MatrixLowerTriangle<D,N>& resize(const Dimensions dims) {
    ...
    MatrixLowerTriangle<D,N>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    MatrixLowerTriangle<D,N>& transpose(void) {
      // TODO: implement
      return *this;
    }
    ...
    typename std::enable_if<is_complex<T>{}, MatrixLowerTriangle<D,N>& >::type adjoint() {
      return *this;
    }
    ...
    MatrixLowerTriangle<D,N>& set(const Vector<D>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    }
    MatrixLowerTriangle<D,N>& operator=(const Vector<D>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    ...
    template <class X>
    MatrixLowerTriangle<D,N>& operator=(const TensorR<X,D,D,1,1>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    ...
    template <class X>
    MatrixLowerTriangle<D,N>& operator=(const TensorR<X,D,D,1,2>& A) {
      const index_type NN = A.dims()[0];
    ...
    MatrixLowerTriangle<D,N>& operator=(const D& value) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = value;
      }
      return *this;
    ...
    MatrixLowerTriangle<D,N>& operator=(const MatrixLowerTriangle<D,N>& b) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = b[k];
      }
      return *this;
  ...
  // NOTE: in-place
    ...
    template< typename T=D >
    typename std::enable_if<is_complex<T>{},  MatrixLowerTriangle<D,N>& >::type conj() {
      return *this;
    }
  ...
  friend std::ostream& operator<<(std::ostream &stream, const MatrixLowerTriangle<D,N>& m) {
    using namespace display;
  ...
  //template <class D>	
  friend inline std::istream& operator>>(const std::string s,  MatrixLowerTriangle<D,N>& m2) {	
  ...
  friend std::istream& operator>>(std::istream& stream,  MatrixLowerTriangle<D,N>& m2) {	
    return stream;
  }
