...
namespace mathq {
  ...
  /********************************************************************
   * MatrixVandermonde<D        -- variable size matrix (valarray)
   *                        D  = type for elements
   * MatrixVandermonde<D,NR>    -- fixed number of rows (valarray)
   *                        NR = number of rows
   * MatrixVandermonde<D,NR,NC> -- fixed number of rows and cols (array)
   *                        NC = number of cols
   ********************************************************************  
   */
  ...
  public:
    ...
    typedef MatrixVandermonde<D,NR,NC> XType;
    typedef D EType;
    typedef D DType;
  ...
  private:
    ...
    static_assert(NumberType<D>::value,
                  "class MatrixVandermonde can only have numbers as elements, ie not vectors, matrices etc.");
  ...
  public:
    ...
    // -------------------  DEFAULT  CONSTRUCTOR --------------------
    explicit MatrixVandermonde<D,NR,NC>() 
    {
      size_t NN = NR*NC;
      resize(NR,NC);
      *this = 0;
    ...
    // -------------------  D value --------------------
    explicit MatrixVandermonde<D,NR,NC>(const D& value) 
    {
      size_t NN = NR*NC;
      resize(NR,NC);
      *this = value;
    ...
    explicit MatrixVandermonde<D,NR,NC>(const Vector<D>& v) 
    {
      resize(NR,NC);  
      *this = v;
    }
    ...
    template<size_t NN = NR*NC, EnableIf<NN == 0> = 0>
      ...
      explicit MatrixVandermonde<D,NR,NC>(const Vector<D>& v, const index_type Nc) 
    {
    ...
    explicit MatrixVandermonde<D,NR,NC>(const size_type Nr, const size_type Nc) {
      resize(Nr,Nc);
      *this = 0;
    }
    ...
    explicit MatrixVandermonde<D,NR,NC>(const size_type Nr, const size_type Nc, const D& value) {
      resize(Nr,Nc);
      *this = value;
    }
    ...
    MatrixVandermonde<D,NR,NC>&  resize(const int Nr, const int Nc) {
      Nrows_ = NR;
      Ncols_ = NC;
    ...
    MatrixVandermonde<D,NR,NC>& resize(const Dimensions dims) {
    ...
    MatrixVandermonde<D,NR,NC>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    MatrixVandermonde<D,NR,NC>& reshape(const size_type nr, const size_type nc) { 
    ...
    MatrixVandermonde<D,NR,NC>& transpose(void) { 
      return *this;
    }
    ...
    typename std::enable_if<is_complex<T>{}, MatrixVandermonde<D,NR,NC>& >::type adjoint() {
      return *this;
    }
    ...
    MatrixVandermonde<D,NR,NC>& set(const Vector<D>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    }
    MatrixVandermonde<D,NR,NC>& operator=(const Vector<D>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    ...
    MatrixVandermonde<D,NR,NC>& operator=(const D& value) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = value;
      }
      return *this;
    ...
    MatrixVandermonde<D,NR,NC>& operator=(const MatrixVandermonde<D,NR,NC>& b) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = b[k];
      }
      return *this;
  ...
  // NOTE: in-place
    ...
    template< typename T=D >
    typename std::enable_if<is_complex<T>{},  MatrixVandermonde<D,NR,NC>& >::type conj() {
      return *this;
    }
  ...
  inline std::string classname() const {
    ...
    std::string s = "MatrixVandermonde";		
  ...
  friend std::ostream& operator<<(std::ostream &stream, const MatrixVandermonde<D,NR,NC>& m) {
    using namespace display;
  ...
  //template <class D>	
  friend inline std::istream& operator>>(const std::string s,  MatrixVandermonde<D,NR,NC>& m2) {	
  ...
  friend std::istream& operator>>(std::istream& stream,  MatrixVandermonde<D,NR,NC>& m2) {	
    return stream;
  }
