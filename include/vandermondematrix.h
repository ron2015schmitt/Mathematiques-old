#ifndef MATHQ__VANDERMONDEMATRIX_H
#define MATHQ__VANDERMONDEMATRIX_H 1
...
namespace mathq {
  ...
  /********************************************************************
   * VandermondeMatrix<D        -- variable size matrix (valarray)
   *                        D  = type for elements
   * VandermondeMatrix<D,NR>    -- fixed number of rows (valarray)
   *                        NR = number of rows
   * VandermondeMatrix<D,NR,NC> -- fixed number of rows and cols (array)
   *                        NC = number of cols
   ********************************************************************  
   */
  ...
  class VandermondeMatrix : public TensorRW<VandermondeMatrix<D,NR,NC>,D,D,1,2>{
  ...
  public:
    ...
    typedef VandermondeMatrix<D,NR,NC> XType;
    typedef D EType;
    typedef D DType;
  ...
  private:
    ...
    static_assert(NumberType<D>::value,
                  "class VandermondeMatrix can only have numbers as elements, ie not vectors, matrices etc.");
  ...
  public:
    ...
    // -------------------  DEFAULT  CONSTRUCTOR --------------------
    explicit VandermondeMatrix<D,NR,NC>() 
    {
      size_t NN = NR*NC;
      resize(NR,NC);
      *this = 0;
    ...
    // -------------------  D value --------------------
    explicit VandermondeMatrix<D,NR,NC>(const D& value) 
    {
      size_t NN = NR*NC;
      resize(NR,NC);
      *this = value;
    ...
    explicit VandermondeMatrix<D,NR,NC>(const Vector<D>& v) 
    {
      resize(NR,NC);  
      *this = v;
    }
    ...
    template<size_t NN = NR*NC, EnableIf<NN == 0> = 0>
      ...
      explicit VandermondeMatrix<D,NR,NC>(const Vector<D>& v, const index_type Nc) 
    {
    ...
    explicit VandermondeMatrix<D,NR,NC>(const size_type Nr, const size_type Nc) {
      resize(Nr,Nc);
      *this = 0;
    }
    ...
    explicit VandermondeMatrix<D,NR,NC>(const size_type Nr, const size_type Nc, const D& value) {
      resize(Nr,Nc);
      *this = value;
    }
    ...
    VandermondeMatrix<D,NR,NC>&  resize(const int Nr, const int Nc) {
      Nrows_ = NR;
      Ncols_ = NC;
    ...
    VandermondeMatrix<D,NR,NC>& resize(const Dimensions dims) {
    ...
    VandermondeMatrix<D,NR,NC>& resize(const std::vector<Dimensions>& deepdims_new) {
    ...
    VandermondeMatrix<D,NR,NC>& reshape(const size_type nr, const size_type nc) { 
    ...
    VandermondeMatrix<D,NR,NC>& transpose(void) { 
      return *this;
    }
    ...
    typename std::enable_if<is_complex<T>{}, VandermondeMatrix<D,NR,NC>& >::type adjoint() {
      return *this;
    }
    ...
    VandermondeMatrix<D,NR,NC>& set(const Vector<D>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    }
    VandermondeMatrix<D,NR,NC>& operator=(const Vector<D>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    ...
    template <class X>
    VandermondeMatrix<D,NR,NC>& operator=(const TensorR<X,D,D,1,1>& v) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = v[k];
      }
      return *this;
    ...
    VandermondeMatrix<D,NR,NC>& operator=(const D& value) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = value;
      }
      return *this;
    ...
    VandermondeMatrix<D,NR,NC>& operator=(const VandermondeMatrix<D,NR,NC>& b) {
      for (index_type k = 0; k < data_.size(); k++) {
	data_[k] = b[k];
      }
      return *this;
    ...
    VandermondeMatrix<D,NR,NC>&  roundzero(FType tolerance = Helper<FType>::tolerance) {
      return *this;
    }
  ...
  // NOTE: in-place
    ...
    template< typename T=D >
    typename std::enable_if<is_complex<T>{},  VandermondeMatrix<D,NR,NC>& >::type conj() {
      return *this;
    }
  ...
  inline std::string classname() const {
    ...
    std::string s = "VandermondeMatrix";		
  ...
  friend std::ostream& operator<<(std::ostream &stream, const VandermondeMatrix<D,NR,NC>& m) {
    using namespace display;
  ...
  //template <class D>	
  friend inline std::istream& operator>>(const std::string s,  VandermondeMatrix<D,NR,NC>& m2) {	
  ...
  friend std::istream& operator>>(std::istream& stream,  VandermondeMatrix<D,NR,NC>& m2) {	
    return stream;
  }
